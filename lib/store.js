...
function () {
    ...
    Object.entries(reducers).forEach(function (_ref5) {
      ...
      switch (typeof reducer) {
        case 'function':
          ...
          if (stateKey) {
            if (typeof parentState[stateKey] === 'object') {
              updateState = function updateState(change) {
            ...
            storage[reducerName] = function () {
              ...
              updateState(reducer.apply(void 0, [getState()].concat(args)));
        ...
        case 'object':
          ...
          _this5._installReducers(reducer, storage[reducerName], childState, state, function () {
  ...
  _proto._installListeners = function _installListeners(listeners, storage, state, store, prefix) {
    ...
    var _loop = function _loop(i, listenersLen) {
      var contextName = listenerKeys[i]; // list -> counter/todo
      ...
      var _loop2 = function _loop2(j, contextLen) {
        var targetName = contextKeys[j]; // counter/todo -> inc/another
        ...
        var _loop3 = function _loop3(k, targetLen) {
          var reducerName = targetKeys[k]; // inc/another -> toggle
